...
base_dir       = 'C:\Users\ansbel\Documents\GitHub\TriCo\data\external\music_listening\part2\';
...
for f = 1:length(mat_files)
    ...
    for k = 1:length(sig_gl)
        ...
        ax1 = nexttile(t,1); title(ax1, 'Spatial Filter (W)'); topo.avg = wx; cfg_topo.figure = ax1; ft_topoplotER(cfg_topo, topo); 
        ax2 = nexttile(t,2); title(ax2, 'Spatial Pattern (A)'); topo.avg = ax; cfg_topo.figure = ax2; ft_topoplotER(cfg_topo, topo); 
    ...
    for i = 1:top_N
        ...
        row = (i-1)*5;
        ax_wx = nexttile(t2, row + 1); topo.avg = wx; cfg_topo.figure = ax_wx; ft_topoplotER(cfg_topo, topo); if i == 1, title('Filter'); end
        ax_ax = nexttile(t2, row + 2); topo.avg = ax; cfg_topo.figure = ax_ax; ft_topoplotER(cfg_topo, topo); if i == 1, title('Pattern'); end
    ...
    %% =====================================================================
    % РИСУНОК 9 (НОВЫЙ): UMAP, РАСКРАШЕННЫЙ ПО МОЩНОСТИ ЛУЧШЕГО ИСТОЧНИКА
    % =====================================================================
    [~, max_lin_idx] = max(abs(corrs_true(:)));
    ...
    S_best = (S_best - mean(S_best)) / std(S_best); % Нормализация для красивой палитры
