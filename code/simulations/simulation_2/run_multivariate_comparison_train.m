...
% 4D-Массивы для метрик: [nMC, nSteps, nMethods, Ndistr]
...
for mc_idx = 1:nMC
    ...
    % Фиксируем тестовые выборки
    z_test = z_epo(:, train_epochs_range(end)+1:Ts);
    z_multidim_test = z_multidim(:, train_epochs_range(end)+1:Ts);
    nTest = size(z_test, 2);
    ...
    Covs_test = Covs_all(:,:, train_epochs_range(end)+1:Ts);
    
    % Временные матрицы для parfor
    f_test_local = zeros(nSteps, nMethods, Ndistr); 
    p_corr_local = zeros(nSteps, nMethods, Ndistr);
    ...
    parfor (step_idx = 1:nSteps, 4)
        ...
        z_multidim_train = z_multidim(:, 1:nTrain);
        % Ковариации пересчитывать не надо, просто берем срез
        Covs_train = Covs_all(:,:, 1:nTrain);
        
        % Временные буферы для текущего шага
        ...
        for m_idx = 1:nMethods
            ...
            for t = 1:min(Nextract, Ndistr)
                ...
                if max_val == -1, break; end % Матрица замаскирована
                ...
                % Маскируем уже привязанный фильтр и истинный источник
                corr_mat(best_ext, :) = -1;
                corr_mat(:, best_tar) = -1;
            end
        end
        ...
        f_test_local(step_idx, :, :) = f_tmp;
        p_corr_local(step_idx, :, :) = p_tmp;
        z_corr_local(step_idx, :, :) = z_tmp;
    end
...
for src_i = 1:Ndistr
    ...
    if src_i == 1, legend('Location', 'southeast', 'FontSize', 10); end
    ...
    if src_i == Ndistr
        subplot(Ndistr, 3, (src_i-1)*3 + 1); xlabel('Number of training epochs', 'FontSize', 11);
        subplot(Ndistr, 3, (src_i-1)*3 + 2); xlabel('Number of training epochs', 'FontSize', 11);
